...
namespace ILFramework.HotClass
{
    ...
    public class TD6732Part2
    {
        ...
        void Start(object o)
        {
            ...
            //_jugle = new bool[clickBox.transform.childCount];
            //Return = curTrans.Find("returnClick").gameObject;
        ...
        //        SpineManager.instance.DoAnimation(obj.transform.GetChild(0).gameObject, obj.transform.GetChild(0).gameObject.name, false);
        //        obj.transform.SetAsLastSibling();
        //        mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, Convert.ToInt32(obj.name), null,
        //            () => { _canRenturn = true; Return.SetActive(true); }
        //            ));
        //        temp = obj.name;
        //    }
        //}
        ...
        //        SpineManager.instance.DoAnimation(clickBox.transform.Find(temp).GetChild(0).gameObject, clickBox.transform.Find(temp).GetChild(0).gameObject.name + "2", false,
        //            () =>
        //            { _canClick = true; obj.SetActive(false); _jugle[Convert.ToInt32(temp) - 1] = true; Jugle(); }
        //            );
        ...
        private void GameInit()
        {
            ...
            //for (int i = 0; i < clickBox.transform.childCount; i++)
            //{
            //    SpineManager.instance.DoAnimation(clickBox.transform.GetChild(i).GetChild(0).gameObject, clickBox.transform.GetChild(i).GetChild(0).gameObject.name + "3", false);
            //}
        ...
        void GameStart()
        {
            ...
            mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 0, null,
                () =>
                {
                    _canClick = true; mask.SetActive(false); bd.SetActive(false);_canMove = true;
                }
                ));
        }
        ...
        void TDGameInit()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                SpinePage.GetChild(1).GetChild(i).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
                SpineManager.instance.DoAnimation(SpinePage.GetChild(1).GetChild(i).gameObject, SpinePage.GetChild(1).GetChild(i).GetChild(0).name+"3", false);
            }
            SpinePage.GetChild(0).GetChild(0).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
            SpineManager.instance.DoAnimation(SpinePage.GetChild(0).GetChild(0).gameObject,SpinePage.GetChild(0).GetChild(0).GetChild(0).gameObject.name+"3",false);
        ...
        void TDGameStart()
        {
            //mask.SetActive(true);
            //dbd.SetActive(true);
            //mono.StartCoroutine(SpeckerCoroutine(dbd, SoundManager.SoundType.COMMONVOICE, 0, null,
            //    () => { }
            //    ));
        ...
        void TalkClick()
        {
            ...
            switch (talkIndex)
            {
                case 1:
                    ...
                    mask.SetActive(true);
                    bd.SetActive(true);
                    mono.StartCoroutine(SpeckerCoroutine(bd,SoundManager.SoundType.VOICE,5));
        ...
        private void SlideSwitchPage(GameObject rayCastTarget)
        {
            ...
            UIEventListener.Get(rayCastTarget).onUp = upData =>
            {
                ...
                if (dis > 300)
                {
                    if (!isRight)
                    {
                        if (curPageIndex <= 0 || !_canMove)
                            return;
                        _canMove = false;
                        _canClick = false;
                        SetMoveAncPosX(1, 1,
                            () => { _canMove = true;_canClick = true; }
                            );
                    }
                    else
                    {
                        if (curPageIndex >= SpinePage.childCount - 1 || !_canMove)
                            return;
                        _canClick = false;
                        _canMove = false;
                        SetMoveAncPosX(-1, 1,
        ...
        private void OnClickBtnRight(GameObject obj)
        {
            ...
            _canClick = false;
            BtnPlaySound();
            SetMoveAncPosX(-1, 1f, () => { _canMove = true; _canClick = true; });
        ...
        private void OnClickBtnBack(GameObject obj)
        {
            ...
            {
                obj.SetActive(false); _canClick = true; _canMove = true;
                if(tem.name=="0")
                {
                    jugle1 = true;
                    Jugle();
                ...
                {
                    _jugle[Convert.ToInt32(tem.name)-1] = true;
                    Jugle();
                }
                /*_jugle[Convert.ToInt32(tem.transform.GetChild(0).GetChild(0).gameObject.name)] = true; Jugle();*/
            });
        }
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            if (tem.transform.parent.name == "2")
            {
            ...
            SpineManager.instance.DoAnimation(tem, obj.name, false, () =>
            {
                ...
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, Convert.ToInt32(tem.name) + 1, null,
                    () =>
                    {
