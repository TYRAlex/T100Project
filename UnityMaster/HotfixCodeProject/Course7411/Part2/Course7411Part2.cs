...
namespace ILFramework.HotClass
{
    public class Course7411Part2
    {
        ...
        void LoadGame()
        {
            ...
            juggle1 = curTrans.Find("juggle1").gameObject;
            juggle.SetActive(false);
            juggle1.SetActive(false);
            ...
            curTrans.Find("arrow1").gameObject.SetActive(false);

            curTrans.Find("lastJuggle").gameObject.SetActive(true);
            curTrans.Find("lastJuggle1").gameObject.SetActive(true);

            Util.AddBtnClick(curTrans.Find("lastJuggle").gameObject, OnClickLastJuggle); ;
            Util.AddBtnClick(curTrans.Find("lastJuggle1").gameObject, OnClickLastJuggle); ;
            ...
            for (int i = 0; i < juggle1.transform.childCount; ++i)
            {
                GameObject obj = juggle1.transform.GetChild(i).gameObject;
                obj.SetActive(true);
                Util.AddBtnClick(obj, EnlargeImage);
            }
        ...
        void GameInit()
        {
            ...
            bigJuggle.transform.Find("Background").GetRectTransform().sizeDelta = new Vector2(514, 729);
        }
        ...
        private void OnClickLastJuggle(GameObject obj)
        {
            mask.gameObject.SetActive(false);
            SoundManager.instance.PlayClip(9);
            curTrans.Find("lastJuggle").gameObject.SetActive(false);
            curTrans.Find("lastJuggle1").gameObject.SetActive(false);

            bool islastJuggle = obj.name == "lastJuggle";
            ...
            {
                curTrans.Find("arrow").gameObject.SetActive(false);
                curTrans.Find("arrow1").gameObject.SetActive(true);

                juggle.SetActive(false);
                juggle1.SetActive(true);
                SoundManager.instance.ShowVoiceBtn(true);
            }
        }
        ...
        void EnlargeImage(GameObject obj)
        {
            ...
            {
                bigJuggle.transform.DOScale(Vector3.one * 1.2f, 0.35f).SetEase(Ease.InOutSine).OnComplete(() =>
                {
                    if (isChange)
                    {
                        ChangEnlargeImage(() =>
