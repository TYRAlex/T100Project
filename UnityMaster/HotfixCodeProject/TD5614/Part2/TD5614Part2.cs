...
namespace ILFramework.HotClass
{
    ...
    public class TD5614Part2
    {
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(tem, obj.name, false, () => { SpineManager.instance.DoAnimation(obj.transform.parent.gameObject, obj.name + "4", false, () => {
                isPressBtn = true;
                btnBack.SetActive(true);
                mono.StartCoroutine(SpeckerCoroutine(SoundManager.SoundType.VOICE, int.Parse(obj.transform.GetChild(0).name), null, () =>
                {
                    if ((flag & (1 << int.Parse(obj.transform.GetChild(1).name))) == 0)
                    {
                        flag += 1 << int.Parse(obj.transform.GetChild(1).name);
                    }
                    isPressBtn = false;
                }));
            }); });
        ...
        void GameStart()
        {
            ...
            mono.StartCoroutine(SpeckerCoroutine(SoundManager.SoundType.VOICE, 0, null, () => {isPlaying = false;}));
        }
        ...
        private void SlideSwitchPage(GameObject rayCastTarget)
        {
            ...
            {
                float dis =  Mathf.Abs(upData.position.x - _prePressPos.x);
