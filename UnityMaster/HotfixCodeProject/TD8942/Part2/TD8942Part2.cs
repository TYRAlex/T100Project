    ...
    public class TD8942Part2
    {
        ...
        private void OnClickBtnBack(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(tem, tem.transform.GetChild(0).name + "2", false, () =>
            {
                ...
                if (flag >= (Mathf.Pow(2, SpinePage.childCount / 2) - 1) && !isMiddle)
                {
                ...
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 7,
                    ...
                    () =>
                    {
                        ...
                        if (curPageIndex < 3)
                        {
                            ...
                            isPlaying = false;
                            SetMoveAncPosX(-1 * (3 - curPageIndex));
                        }
                        else
                        {
                            isPlaying = false;
        ...
        private void SetMoveAncPosX(int LorR, float duration = 1f, Action callBack = null)
        {
            ...
            {
                LRBtnUpdate(); callBack?.Invoke(); isPlaying = false; isPressBtn = false;                
                if (flag >= (Mathf.Pow(2, SpinePage.childCount / 2) - 1) && !isMiddle)
                {
                    SoundManager.instance.ShowVoiceBtn(true);
                }
