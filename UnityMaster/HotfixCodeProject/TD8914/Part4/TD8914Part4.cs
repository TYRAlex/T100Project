...
namespace ILFramework.HotClass
{
    ...
    public class TD8914Part4
    {
        ...
        {
            if (curPageIndex >= 2 || isPlaying || isPressBtn)
                return;
            isPressBtn = true;
            BtnPlaySound();
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            if(obj.name=="c")
            {
                mono.StartCoroutine(SpeckerCoroutine(SoundManager.SoundType.VOICE,bd,2,null,()=> { isPressBtn = false; }));
            }
            if (obj.name == "d")
            {
                mono.StartCoroutine(SpeckerCoroutine(SoundManager.SoundType.VOICE, bd, 1, null, () => { isPressBtn = false; }));
            }
            if (obj.name == "e")
            {
                mono.StartCoroutine(SpeckerCoroutine(SoundManager.SoundType.VOICE, bd, 3, null, () => { isPressBtn = false; }));
            }
        ...
        void TalkClick()
        {
            ...
            if (talkIndex == 1)
            {
                mask.Show();
                bd.SetActive(true);
                mono.StartCoroutine(SpeckerCoroutine(SoundManager.SoundType.VOICE, bd, 4, null, () =>
                {
