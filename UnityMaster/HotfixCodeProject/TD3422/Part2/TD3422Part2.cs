...
namespace ILFramework.HotClass
{
    ...
    public class TD3422Part2
    {
        ...
        void Start(object o)
        {
            ...
            e4rs = SpinePage.GetGameObject("SpineImage").GetComponentsInChildren<Empty4Raycast>(true);
            for (int i = 0, len = e4rs.Length; i < len; i++)
            {
                Util.AddBtnClick(e4rs[i].gameObject, OnClickShow);
            }
            SpineShow = curTrans.Find("PageBar/MaskImg/SpinePage/SpineShow");
        ...
        private void OnClickBtnBack(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(tem, tem.transform.GetChild(0).name + "2", false, () =>
            {
                ...
                isPlaying = false;
                isPressBtn = false;
                if (flag == (Mathf.Pow(2, SpinePage.Find("SpineImage").childCount) - 1) && !isEnd)
                {
                    SoundManager.instance.ShowVoiceBtn(true);
                }
            });
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(tem, obj.name, false, () =>
            {
                ...
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, int.Parse(obj.transform.GetChild(0).name) + 11, null, () =>
                {
                    //用于标志是否点击过展示板
        ...
        private void GameInit()
        {
            ...
            for (int i = 0; i < SpinePage.GetGameObject("SpineImage").transform.childCount; i++)
            {
                SpineManager.instance.DoAnimation(SpinePage.GetGameObject("SpineImage").transform.GetChild(i).gameObject, SpinePage.GetGameObject("SpineImage").transform.GetChild(i).gameObject.name + "3", false);
            }
        }
        ...
        void TalkClick()
        {
            ...
            if (talkIndex == 1)
            {
                ...
                mask.SetActive(true);
                bd.SetActive(true);
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 10, null,
                () => 
                { 
            ...
            if (talkIndex == 2)
            {
                mask.SetActive(true);
                bd.SetActive(true);
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 14, null, null));
            }
            talkIndex++;
        }
