...
namespace ILFramework.HotClass
{
    ...
    public class TD8932Part2
    {
        ...
        private void ReturnEvent(GameObject obj)
        {
            if(_canRenturn)
            {
                ...
                SpineManager.instance.DoAnimation(clickBox.transform.Find(temp).GetChild(0).gameObject, clickBox.transform.Find(temp).GetChild(0).gameObject.name+ "2",false,
                    ()=>
                    { _canClick = true;obj.SetActive(false);
                        _jugle[Convert.ToInt32(temp) - 1] = true;
                        Jugle();
                    }
                    );
        ...
        private void GameInit()
        {
            ...
            for (int i = 0; i < clickBox.transform.childCount; i++)
            {
                clickBox.transform.GetChild(i).GetChild(0).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
        ...
        IEnumerator Wait(Action callback = null)
        {
        ...
        void GameStart()
        {
            ...
            mono.StartCoroutine(SpeckerCoroutine( bd,SoundManager.SoundType.VOICE,0,null,
                ()=>
                {
                    _canClick = true;mask.SetActive(false);bd.SetActive(false);
                }
                ));
        }
