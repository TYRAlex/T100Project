...
namespace ILFramework.HotClass
{
    ...
    public class Course737Part4
    {
        ...
        void Start(object o)
        {
            ...
            #region 需要进行修改的部分
            //添加选择题类型
            _questionType = new QuestionType[3] { QuestionType.Judge, QuestionType.Judge, QuestionType.Single };
            ...
            tittle = curTrans.Find("tittle").gameObject;
            unClickMask = curTrans.Find("unClickMask").gameObject;

            GameInit();

            unClickMask.SetActive(true);
            float _time = SoundManager.instance.PlayClip(SoundManager.SoundType.VOICE, 0);
            mono.StartCoroutine(WaitFor(_time, ()=> unClickMask.SetActive(false)));
        }
                    ...
                    if (_questionType[_level] != QuestionType.Multiple)
                    {
                        ...
                        if (answerIndex == 10)
                        {
                            ...
                            SpineManager.instance.DoAnimation(head, "31", false, () =>
                            {
                                ...
                                float _time = 0;
                                if (_level == 0) _time = SoundManager.instance.PlayClip(SoundManager.SoundType.VOICE, 1);
                                if (_level == 1) _time = SoundManager.instance.PlayClip(SoundManager.SoundType.VOICE, 2);
                        ...
                        else
                        {
                            ...
                            {
                                SpineManager.instance.DoAnimation(head2, "1", true);
