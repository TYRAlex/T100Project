...
namespace ILFramework.HotClass
{
    ...
    public class Course9212Part2
    {
        ...
        void Start(object o)
        {
            ...
            #region 需要进行修改的部分
            //添加选择题类型
            _questionType = new QuestionType[3] { QuestionType.Single, QuestionType.Single, QuestionType.Single };

            //添加正确选项
            _answerDic = new Dictionary<int, int[]>();
            _answerDic.Add(0, new int[1] { 0 });
            _answerDic.Add(1, new int[1] { 1 });
        ...
        {
            curTrans.Find("show").gameObject.SetActive(false);
            talkIndex = 1;
            _level = 0;
            _canClick = true;
            _maskC.Hide();
        ...
        void GameStart()
        {
            ...
            bell.SetActive(true);
            curTrans.Find("show").gameObject.SetActive(true);
            curTrans.Find("show").SetAsLastSibling();
            mono.StartCoroutine(SpeckerCoroutine(bell,SoundManager.SoundType.VOICE,0,null,
                ()=>
                {
                    ...
                    curTrans.Find("show").gameObject.SetActive(false);
                    _maskC.transform.SetAsLastSibling();
                    _maskC.Hide();
                    bell.Hide();
                }
                ));
