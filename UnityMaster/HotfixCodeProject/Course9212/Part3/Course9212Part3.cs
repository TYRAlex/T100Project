...
namespace ILFramework.HotClass
{
    ...
    public class Course9212Part3
    {
        ...
        void Start(object o)
        {
            ...
            #region 需要进行修改的部分
            //添加选择题类型
            _questionType = new QuestionType[10] { QuestionType.Multiple, QuestionType.Single, QuestionType.Multiple, QuestionType.Single, QuestionType.Single , QuestionType.Single , QuestionType.Single , QuestionType.Single , QuestionType.Single , QuestionType.Single };

            //添加正确选项
            _answerDic = new Dictionary<int, int[]>();
            _answerDic.Add(0, new int[4] { 0,1,2,3 });
            _answerDic.Add(1, new int[1] {  2 });
            _answerDic.Add(2, new int[2] { 0,1});
            _answerDic.Add(3, new int[1] { 0 });
            _answerDic.Add(4, new int[1] { 1 });
            _answerDic.Add(5, new int[1] { 0 });
            _answerDic.Add(6, new int[1] { 3 });
            _answerDic.Add(7, new int[1] { 1 });
            _answerDic.Add(8, new int[1] { 1 });
            _answerDic.Add(9, new int[1] { 1 });
            #endregion
        ...
        void GameStart()
        {
            //mono.StartCoroutine(Move(curTrans.Find("RawImage").gameObject));
        }
        ...
        void ShowCurQuestion()
        {
            ...
            for (int i = 0; i < _clickArray[_level].transform.childCount; i++)
            {
                ...
                tra.GetComponent<CanvasGroup>().alpha = 0;  //加入CanvasGroup来控制alpha值，防止上下题切换动画闪的bug
                SpineManager.instance.DoAnimation(tra.gameObject, "0", false);

                mono.StartCoroutine(WaitCoroutine(
                () =>
                {
                    for (int j = 0; j < _clickArray[_level].transform.childCount; j++)
                    {
                        _clickArray[_level].transform.GetChild(j).GetChild(0).GetComponent<CanvasGroup>().alpha = 1;
                    }
                }, 0.2f));
            }
        }
