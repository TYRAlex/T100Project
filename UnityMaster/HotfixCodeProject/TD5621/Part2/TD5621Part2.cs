...
namespace ILFramework.HotClass
{
    ...
    public class TD5621Part2
    {
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            if (obj.name == "f") index = 1;
            if (obj.name == "g") index = 2;
            if (obj.name == "h") index = 3;
            SpineManager.instance.DoAnimation(tem, obj.name, false, () =>
            {
                isPressBtn = true;
                btnBack.SetActive(true);
                //播放对应语音
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, index, null, () =>
                {
                    //用于标志是否点击过展示板
                    isPressBtn = false;
                }));
