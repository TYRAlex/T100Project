...
namespace ILFramework.HotClass
{
    ...
    public class TD6743Part1
    {
        ...
        private void GameInit()
        {
            ...
            pageBar.transform.localPosition = new Vector3(1920, 0, 0);
            isPlaying = true;
            talkIndex = 1;
            //田丁初始化
            TDGameInit();
        ...
        void TDGameInit()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                SpinePage.GetChild(0).GetChild(i).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
                SpineManager.instance.DoAnimation(SpinePage.GetChild(0).GetChild(i).gameObject, SpinePage.GetChild(0).GetChild(i).GetChild(0).name+"3",false);
            }
            SpinePage.GetChild(1).GetChild(0).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
            SpineManager.instance.DoAnimation(SpinePage.GetChild(1).GetChild(0).gameObject, SpinePage.GetChild(1).GetChild(0).GetChild(0).name + "3", false);
                ...
                case 2:
                    ...
                    mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 9, null,
                        () =>
                        {
                            ...
                            pageBar.transform.GetRectTransform().DOAnchorPosX(pageBar.transform.GetRectTransform().anchoredPosition.x - 1920, 1f).OnComplete(() => { _canClick = true; });
                            _canMove = true;
                            LRBtnUpdate();
                        }
                        ));
        ...
        private void OnClickPlay(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(SpineShow.gameObject, (obj.transform.GetSiblingIndex() + 1).ToString(), false);
        }
        ...
        private void OnClickBtnBack(GameObject obj)
        {
            ...
            {
                if ((temp & (1 << Convert.ToInt32(tem.name))) == 0)
                {
                    temp += 1 << Convert.ToInt32(tem.name);
                }
                if (temp == (Mathf.Pow(2, 4) - 1))
                {
                    SoundManager.instance.ShowVoiceBtn(true);
                }
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            {
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, Convert.ToInt32(tem.name)+10, null,
                    () => {
