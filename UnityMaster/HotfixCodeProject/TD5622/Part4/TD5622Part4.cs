...
namespace ILFramework.HotClass
{
    ...
    public class TD5622Part4
    {
        ...
        void Start(object o)
        {
            ...
            _canClick = true;
            _canReturn = false;
            returnbtn = curTrans.Find("return").gameObject;
            returnbtn.SetActive(false);
            level1 = curTrans.Find("level1").gameObject;
            ani = curTrans.Find("level1/ani").gameObject;
            ...
            Util.AddBtnClick(returnbtn, ReturnEvent);
        ...
        private void ClickEventLevel1(GameObject obj)
        {
            if (_canClick)
            {
                ...
                _returnobj = name;
                level1.transform.Find("ani").Find(name).SetAsLastSibling();
                SpineManager.instance.DoAnimation(level1.transform.Find("ani").Find(name).gameObject, name, false );
            }
        }

        private void ClickJugle(GameObject obj)
        {
            int index = 0;
            ...
            mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, index+1, null,
                () =>
                {
        ...
        private void ReturnEvent(GameObject obj)
        {
            if (_canReturn)
            {
                ...
                SpineManager.instance.DoAnimation(level1.transform.Find("ani").Find(_returnobj).gameObject, _returnobj + "2", false,
                    () =>
                    {
                        returnbtn.SetActive(false);
                        _canClick = true;
        ...
        private void GameInit()
        {
            ...
            SpineManager.instance.DoAnimation(ani.transform.GetChild(0).gameObject, ani.transform.GetChild(0).gameObject.name + "3", false);
            SpineManager.instance.DoAnimation(ani.transform.GetChild(1).gameObject, ani.transform.GetChild(1).gameObject.name + "3", false);
            SpineManager.instance.DoAnimation(ani.transform.GetChild(2).gameObject, ani.transform.GetChild(2).gameObject.name + "3", false);
            talkIndex = 1;
            //田丁初始化
            TDGameInit();
        }
