...
namespace ILFramework.HotClass
{
    ...
    public class TD5622Part1
    {
        ...
        private GameObject showBox;
        private bool _canClick;
        private int showIndex;
        private bool[] jugleAll;
        ...
        void Start(object o)
        {
            ...
            showBox = curTrans.Find("showBox").gameObject;
            jugleAll = new bool[clickBox.transform.Find("yes").childCount];
            _canClick = true;
            for (int i = 0; i < clickBox.transform.Find("yes").childCount; i++)
            {
                Util.AddBtnClick(clickBox.transform.Find("yes").GetChild(i).gameObject, ClickYesEvent);
            }
            for (int i = 0; i < clickBox.transform.Find("no").childCount; i++)
            {
                Util.AddBtnClick(clickBox.transform.Find("no").GetChild(i).gameObject, ClickNoEvent);
            }
            GameInit();
        }

        private void BeginBallOut(float time, Action callback = null)
        {
            mono.StartCoroutine(ieballbegin(time, callback));
        }

        IEnumerator ieballbegin(float time, Action callback = null)
        {
            for (int i = 0; i < clickBox.transform.Find("yes").childCount; i++)
            {
                SpineManager.instance.DoAnimation(clickBox.transform.Find("yes").GetChild(i).GetChild(0).gameObject, clickBox.transform.Find("yes").GetChild(i).GetChild(0).name, true);
                yield return new WaitForSeconds(time);

            }
            for (int i = 0; i < clickBox.transform.Find("no").childCount; i++)
            {
                SpineManager.instance.DoAnimation(clickBox.transform.Find("no").GetChild(i).GetChild(0).gameObject, clickBox.transform.Find("no").GetChild(i).GetChild(0).name, true);
                yield return new WaitForSeconds(time);
            }
            callback?.Invoke();
            yield break;
        }
        private void ClickYesEvent(GameObject obj)
        {
            if (_canClick)
            {
                _canClick = false;
                ...
                SoundManager.instance.PlayClip(SoundManager.SoundType.COMMONVOICE, i, false);
                ClickYesJugle(obj);
                SpineManager.instance.DoAnimation(obj.transform.GetChild(0).gameObject, obj.name + "2", false,
                    () =>
                    {
                        SpineManager.instance.DoAnimation(obj.transform.GetChild(0).gameObject, obj.name + "3", false,
                            () =>
                            {
                                SpineManager.instance.DoAnimation(showBox.transform.Find(obj.name).gameObject, "t" + showIndex, false); _canClick = true; obj.SetActive(false);
                                jugleAll[showIndex - 1] = true; Jugleall();
                            }
                            );
                    }
                    );
        ...
        private void ClickYesJugle(GameObject obj)
        {
            switch (obj.name)
            {
                case "y-a":
        ...
        private void ClickNoEvent(GameObject obj)
        {
            if (_canClick)
            {
                ...
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.COMMONVOICE, i, null,
                    () =>
                    {
                        _canClick = true;
                    }
        ...
        {
            for (int i = 0; i < jugleAll.Length; i++)
            {
                if (!jugleAll[i])
                    return;
            }
            ...
            //mask.SetActive(true);
            //mask.transform.Find("successSpine").gameObject.SetActive(true);
            //SpineManager.instance.DoAnimation(mask.transform.Find("successSpine").gameObject, "3-5-z", false);
        }
        ...
        private void GameInit()
        {
            ...
            {
                jugleAll[i] = false;
                clickBox.transform.Find("yes").GetChild(i).gameObject.SetActive(true);
            }
            for (int i = 0; i < showBox.transform.childCount; i++)
            {
                SpineManager.instance.DoAnimation(showBox.transform.GetChild(i).gameObject, "kong", false);
            }
            for (int t = 0; t < clickBox.transform.Find("no").childCount; t++)
            {
                SpineManager.instance.DoAnimation(clickBox.transform.Find("no").GetChild(t).GetChild(0).gameObject, clickBox.transform.Find("no").GetChild(t).gameObject.name, true);
            }
            for (int t = 0; t < clickBox.transform.Find("yes").childCount; t++)
            {
                SpineManager.instance.DoAnimation(clickBox.transform.Find("yes").GetChild(t).GetChild(0).gameObject, clickBox.transform.Find("yes").GetChild(t).gameObject.name, true);
            }
            BeginBallOut(0.1f);
        }
