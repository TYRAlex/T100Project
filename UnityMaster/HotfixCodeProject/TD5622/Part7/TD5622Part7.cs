...
namespace ILFramework.HotClass
{
    ...
    public class TD5622Part7
    {
        ...
        void Start(object o)
        {
            ...
            returnbtn = curTrans.transform.Find("return").gameObject;
            returnbtn.SetActive(false);
            allPage = curTrans.Find("allPage/layout").gameObject;
            _curPageIndex = 0;
            ...
            Util.AddBtnClick(click, ClickEvent);
        ...
        private void ClickEvent(GameObject obj)
        {
            ...
            {
                _canClick = false;
                _returnobj = allPage.transform.GetChild(_curPageIndex).GetChild(0).gameObject.name;
                ClickJugle(obj);
                SoundManager.instance.PlayClip(SoundManager.SoundType.COMMONSOUND, 1, false);
                allPage.transform.GetChild(_curPageIndex).GetChild(0).SetAsLastSibling();
                returnbtn.SetActive(true);
                SpineManager.instance.DoAnimation(allPage.transform.GetChild(_curPageIndex).GetChild(0).gameObject, aniname, false,
                    () =>
                    {
        ...
        private void ReturnEvent(GameObject obj)
        {
            if (_canReturn)
            {
                ...
                SpineManager.instance.DoAnimation(allPage.transform.GetChild(_curPageIndex).GetChild(0).gameObject, _returnobj + "2", false,
                    () =>
                    {
                        returnbtn.SetActive(false);
                        _canClick = true;
        ...
        private void GameInit()
        {
            ...
            _curPageIndex = 0;
            _pageMinIndex = 0;
            _pageMaxIndex = allPage.transform.childCount - 1;
            _moveValue = 1920;
            _duration = 1f;
            ...
            allPage.GetComponent<RectTransform>().localPosition = new Vector2(-960, 0);
            _canClick = true;
            _canReturn = false;

            for (int i = 0; i < allPage.transform.childCount; i++)
            {
                SpineManager.instance.DoAnimation(allPage.transform.GetChild(i).GetChild(0).gameObject, allPage.transform.GetChild(i).GetChild(0).name + "3", false);
            }

            var rect = allPage.transform.GetRectTransform();
            SlideSwitchPage(allPage.transform.parent.gameObject,
                () => { LeftSwitchPage(rect, _moveValue, _duration, () => { _mask.gameObject.Show(); }, () => { _mask.gameObject.Hide();_isSlide = false; }); },
                () => { RightSwitchPage(rect, -_moveValue, _duration, () => { _mask.gameObject.Show(); }, () => { _mask.gameObject.Hide(); _isSlide = false; }); });
        ...
        private void LRBtnUpdate()
        {
            ...
            else
            {
                SpineManager.instance.DoAnimation(leftBtn.transform.parent.gameObject, leftBtn.transform.parent.name, false);
                SpineManager.instance.DoAnimation(rightBtn.transform.parent.gameObject, rightBtn.transform.parent.name, false);
                rightBtn.SetActive(true); leftBtn.SetActive(true);
            }
        }
        ...
        private void SetMoveAncPosX(RectTransform rect, float value, float duration, Action callBack = null)//用得到
        {
            ...
            rect.DOAnchorPosX(value, duration).OnComplete(() => {
                LRBtnUpdate();
                callBack?.Invoke(); });
        ...
        private void OnClickArrows(GameObject go)
        {
            ...
            if (name == "L")
            {
                SpineManager.instance.DoAnimation(leftBtn.transform.parent.gameObject.gameObject, name, false);
            }
