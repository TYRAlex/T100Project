...
namespace ILFramework.HotClass
{
    ...
    public class TD5622Part2
    {
        ...
        void Start(object o)
        {
            ...
            M = curTrans.Find("M").gameObject;
            Return = curTrans.Find("Return").gameObject;
            Level1 = curTrans.Find("Level1").gameObject;
            Level2 = curTrans.Find("Level2").gameObject;
            _ani = curTrans.Find("Level1/Ani").gameObject;
            _allClicklevel1 = new bool[Level1.transform.childCount - 1];
            _allClicklevel2 = new bool[Level2.transform.childCount - 1];
            ...
            {
                if (Level1.transform.GetChild(i).gameObject.name == "Ani")
                    break;
                else
                    Util.AddBtnClick(Level1.transform.GetChild(i).gameObject, ClickEventLevel1);
            }
            for (int i = 0; i < _allClicklevel2.Length; i++)
            {
                _allClicklevel2[i] = false;
            }
            for (int i = 0; i < Level2.transform.childCount; i++)
            {
                if (Level2.transform.GetChild(i).gameObject.name == "Ani")
                    break;
                else
                    Util.AddBtnClick(Level2.transform.GetChild(i).gameObject, ClickEventLevel2);
            }
            Util.AddBtnClick(Return, Returnway);

            M.SetActive(true);
            mask.SetActive(true);
            mask.transform.Find("BD").gameObject.SetActive(true);
            TDGameStartFunc();
        ...
        private void ClickEventLevel2(GameObject obj)
        {
            if (_canClick)
            {
                ...
                string name = obj.name;
                _jugleReturn = name;
                Level2.transform.Find("Ani").Find(name).SetAsLastSibling();
                SpineManager.instance.DoAnimation(Level2.transform.Find("Ani").Find(name).gameObject, name, false);
            }
        }

        private void Returnway(GameObject obj)
        {
            if (_canReturn)
            {
                _canReturn = false;
                SoundManager.instance.PlayClip(SoundManager.SoundType.COMMONSOUND, 2, false);
                SpineManager.instance.DoAnimation(Level2.transform.Find("Ani").Find(_jugleReturn).gameObject, _jugleReturn + "2", false,
                    () =>
                    {
                        _canClick = true;
        ...
        void ClickJugle(GameObject obj)
        {
            int index = 0;
            switch (obj.transform.gameObject.name)
            {
                case "1":
                    index = 0;
                    break;
            ...
            mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, index+2, null,
                () =>
                {
                    _canClick = true;
        ...
        void ClickJugle2(GameObject obj)
        {
            int index = 0;
            switch (obj.transform.gameObject.name)
            {
            ...
            mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, index+10, null,
                () =>
                {
                    M.SetActive(false);
                    Return.SetActive(true);
                    _canReturn = true;
                    _allClicklevel2[index] = true;

                }
                ));
        }
        private void JugleAllClickLevel1()
        {
            bool all = true;
            for (int i = 0; i < _allClicklevel1.Length; i++)
            {
            ...
            else { SoundManager.instance.ShowVoiceBtn(false); }
        }

        private void JugleAllClickLevel2()
        {
            bool all = true;
            for (int i = 0; i < _allClicklevel2.Length; i++)
            {
        ...
        private void GameInit()
        {
            ...
            SoundManager.instance.ShowVoiceBtn(false);
            Level1.transform.localPosition = new Vector2(0, 0);
            Level2.transform.localPosition = new Vector2(1920, 0);

            SpineManager.instance.DoAnimation(_ani, "0", false);
            SpineManager.instance.DoAnimation(Level2.transform.Find("Ani").Find("a").gameObject, "a3", false);
            SpineManager.instance.DoAnimation(Level2.transform.Find("Ani").Find("b").gameObject, "b3", false);
            SpineManager.instance.DoAnimation(Level2.transform.Find("Ani").Find("c").gameObject, "c3", false);
        ...
        void TDGameNextFunc()
        {
            ...
            bd.SetActive(true);
            mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 9, null, () =>
            {
                SoundManager.instance.PlayClip(SoundManager.SoundType.SOUND,1,false);
                mask.SetActive(false); bd.SetActive(false); Level1.transform.DOLocalMove(new Vector2(-1920f, 0), 0.5f); Level2.transform.DOLocalMove(new Vector2(0, 0), 0.5f);
                _canClick = true;
            }
               ));
        }
        ...
        void TDGameStartFunc()
        {
            mono.StartCoroutine(SpeckerCoroutine(bd,SoundManager.SoundType.VOICE,0,null,
                ()=>
                {
                    ...
                    {
                        M.SetActive(false); mask.SetActive(false); bd.SetActive(false);
                    }
             ));
                }
                ));
