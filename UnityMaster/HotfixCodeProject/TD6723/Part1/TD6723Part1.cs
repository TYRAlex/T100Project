...
namespace ILFramework.HotClass
{
    ...
    public class TD6723Part1
    {
        ...
        void Start(object o)
        {
            ...
            //part1 获取物体
            _aniParent = curTrans.GetGameObject("aniList");
            _imageParent = curTrans.GetGameObject("imageList");

            _aniMask = curTrans.GetGameObject("animask");
            _dragerList = new List<mILDrager>();
            _droperList = new List<mILDroper>();

            for (int i = 0; i < _aniParent.transform.childCount; i++)
            {
                var child = _aniParent.transform.GetChild(i).gameObject;
                _dd = _aniParent.transform.GetChild(0).gameObject;
                if (i > 0)
                {
                    _droperList.Add(child.transform.GetChild(0).GetComponent<mILDroper>());
                }
            }

            for (int i = 0; i < _imageParent.transform.childCount; i++)
            {
                var child = _imageParent.transform.GetChild(i).gameObject;
                _dragerList.Add(child.GetComponent<mILDrager>());
            }

            for (int i = 0; i < _dragerList.Count; i++)
            {
                var temp = _dragerList[i];
                temp.SetDragCallback(StartDrag, null, EndDrag);
            }

            for (int i = 0; i < _droperList.Count; i++)
            {
                var temp = _droperList[i];
                temp.SetDropCallBack(After, null, null);
            }
        ...
        private void StartDrag(Vector3 pos, int type, int index)
        {
            ...
            _dragerList[index].GetComponent<Image>().sprite =
                _dragerList[index].GetComponent<BellSprites>().sprites[1];
        }

        private void EndDrag(Vector3 pos, int type, int index, bool istrue)
        {
            _aniMask.Show();
            ...
            if (istrue)
            {
                ...
                if (score == 13)
                {
                    ...
                    {
                        isend = true;
                        var r = _aniParent.transform.GetChild(0).gameObject;
                        PlaySpineAni(r, "r", true, () => { });
            ...
            _dragerList[index].GetComponent<Image>().DOColor(new Color(0, 0, 0, 0), 1f).OnComplete(() =>
            {
                _dragerList[index].DoReset();
                _dragerList[index].GetComponent<Image>().DOColor(new Color(1, 1, 1, 1), 0.01f).OnComplete(() =>
                {
                    _dragerList[index].GetComponent<Image>().sprite =
        ...
        private bool After(int dragType, int dropIndex, int droptype)
        {
        ...
        void SetTrue(int dropIndex)
        {
            Debug.Log("SetTure");
            _droperList[dropIndex].transform.parent.GetComponent<CanvasGroup>().DOFade(0, 1f);
            _droperList[dropIndex].isActived = false;
        }
        ...
        private void GameInit()
        {
            ...
            for (int i = 0; i < _dragerList.Count; i++)
            {
                var child = _dragerList[i];
                child.GetComponent<Image>().sprite = child.GetComponent<BellSprites>().sprites[0];
            }

            for (int i = 0; i < _aniParent.transform.childCount; i++)
            {
                var temp = _aniParent.transform.GetChild(i).gameObject;
                temp.Show();
                if (i == 0)
                {
                    PlaySpineAni(temp, temp.name + "2", true, null);
                }
                else
                {
                    PlaySpineAni(temp, temp.name, true, null);
                }
            }
            ...
            {
                var temp = _droperList[i];
                temp.transform.parent.GetComponent<CanvasGroup>().alpha = 1;
                temp.isActived = true;
            }
        }
        ...
        void GameStart()
        {
            ...
            Debug.Log("Start 2");
            Talk(bd, 0, "bd-daiji","bd-speak",() => { Debug.Log("Start"); },
        ...
        /// <returns></returns>
        IEnumerator SpeckerCoroutine(GameObject speaker,string daiji,string speak, SoundManager.SoundType type, int clipIndex,
        ...
        void TalkClick()
        {
            ...
            switch (talkIndex)
            {
                case 1:
                    //田丁游戏开始方法
                    Talk(bd, 1, "bd-daiji","bd-speak",null, () =>
                    {
                        mask.Hide();
                        bd.Hide();
                    });
        ...
        private void Talk(GameObject target, int index,string daiji,string speak,Action goingEvent = null, Action finishEvent = null)
        {
            target.Show();
            mono.StartCoroutine(SpeckerCoroutine(target,daiji,speak ,SoundManager.SoundType.VOICE, index, goingEvent,
                finishEvent));
        }
        ...
        private void BtnPlaySoundFail(Action method = null)
        {
            ...
            WaitTimeAndExcuteNext(time, () => { method?.Invoke(); });
        }
        ...
        private void OnClickPlay(GameObject obj)
        {
            ...
            mono.StartCoroutine(SpeckerCoroutine(bd, "bd-daiji","bd-speak",SoundManager.SoundType.VOICE, obj.transform.GetSiblingIndex() + 1,
                null, () =>
                {
                    isPlaying = false;
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(tem, obj.name, false, () =>
            {
                SpineManager.instance.DoAnimation(obj.transform.parent.gameObject, obj.name + "3", false, () =>
                {
                    ...
                    mono.StartCoroutine(SpeckerCoroutine(bd, "bd-daiji","bd-speak",SoundManager.SoundType.VOICE,
                ...
                SpineManager.instance.DoAnimation(obj, obj.name, false, () =>
                {
                    ...
                    else
                    {
                        SpineManager.instance.DoAnimation(obj, "kong", false, () =>
                        {
                            ...
                            dbd.SetActive(true);
                            //结束语音
                            mono.StartCoroutine(SpeckerCoroutine(dbd, "daiji","speak",SoundManager.SoundType.VOICE, 2));
                        });
                    }
                });
            }
