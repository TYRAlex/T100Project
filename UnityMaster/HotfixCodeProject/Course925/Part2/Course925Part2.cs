...
namespace ILFramework.HotClass
{
    public class Course925Part2
    {
        ...
        void Start(object o)
        {
            ...
            _jingSpine = curTrans.GetGameObject("Spines/jing");
        ...
        private void OnClicks(GameObject go)
        {
            ...
            PlayCommonSound(1); HideVoiceBtn();

            var name = go.name;
            var contain = _records.Contains(name);
            if (!contain)
                _records.Add(name);

            var isShowVoice = _records.Count == _onClicks.childCount;

            var soundIndex = int.Parse(go.transform.GetChild(0).name);
            var spineName = go.transform.GetChild(1).name;
            var backName = go.transform.GetChild(2).name;
            var voiceIndex = soundIndex - 1;
            
            PlaySequenceSpine(_jingSpine, new List<string> { name, spineName });
            Delay(1.0f, () => {
               var soundTime= PlaySound(soundIndex);
                PlayVoice(voiceIndex);
                Delay(soundTime, Back);
            });
