...
namespace ILFramework.HotClass
{
    ...
    public class TD8933Part1
    {
        ...
        void Start(object o)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                Util.AddBtnClick(level1.transform.GetChild(i).GetChild(0).gameObject,OnClickShow);
                Util.AddBtnClick(level2.transform.GetChild(i).GetChild(0).gameObject, OnClickShow);
            }
        ...
        private void GameInit()
        {
            ...
            SpineShow.transform.GetRectTransform().localPosition = new Vector3(-960, 558, 0);
            level1.transform.localPosition = new Vector3(960,558, 0);
            level2.transform.localPosition = new Vector3(960, 558, 0);
            for (int i = 0; i < 2; i++)
            {
                level1.transform.GetChild(i).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
                level2.transform.GetChild(i).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
                SpineManager.instance.DoAnimation(level1.transform.GetChild(i).gameObject, level1.transform.GetChild(i).GetChild(0).name+"3",false);
                SpineManager.instance.DoAnimation(level2.transform.GetChild(i).gameObject, level2.transform.GetChild(i).GetChild(0).name + "3", false);
            }
            isPlaying = true;
            talkIndex = 1;
            //田丁初始化
                ...
                case 2:
                    ...
                    mono.StartCoroutine(SpeckerCoroutine(bd,SoundManager.SoundType.VOICE,7,null,
                        ()=>
                        {
                            ...
                            bd.SetActive(false);
                            SpineShow.GetRectTransform().DOAnchorPosX(SpineShow.GetRectTransform().anchoredPosition.x - 1920, 1f);
                            level1.transform.GetRectTransform().DOAnchorPosX(level1.transform.GetRectTransform().anchoredPosition.x - 1920, 1f).OnComplete(()=> { _canClick = true; });
                        }
                        ));
                    break;
                case 3:
                    ...
                    mono.StartCoroutine(SpeckerCoroutine(bd,SoundManager.SoundType.VOICE,12,null,
                        ()=>
                        {
                            mask.SetActive(false);
                            bd.SetActive(false);
                            level1.transform.GetRectTransform().DOAnchorPosX(level1.transform.GetRectTransform().anchoredPosition.x - 1920, 1f);
                            level2.transform.GetRectTransform().DOAnchorPosX(level2.transform.GetRectTransform().anchoredPosition.x - 1920, 1f).OnComplete(() => { _canClick = true; });
                            _jugle[0] = false;
                            _jugle[1] = false;
                        }
                        ));
        ...
        private void OnClickBtnBack(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(tem, tem.transform.GetChild(0).name + "2", false, () =>
            {
                ...
                if (Convert.ToInt32(tem.name) == 3 || Convert.ToInt32(tem.name) == 2)
                {
                    _jugle[Convert.ToInt32(tem.name) - 2] = true;
                }
                else
                {
                    _jugle[Convert.ToInt32(tem.name)] = true;
                }
                Debug.LogError(_jugle[0]);
                Debug.LogError(_jugle[1]);
                Jugle();
            });
        }
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            {
                mono.StartCoroutine(SpeckerCoroutine(bd,SoundManager.SoundType.VOICE,Convert.ToInt32(tem.name)+8,null,
                    () => {
