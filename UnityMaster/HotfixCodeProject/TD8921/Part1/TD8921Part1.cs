    ...
    public class TD8921Part1
    {
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            SpineManager.instance.DoAnimation(tem, obj.name, false, () =>
            {
                ...
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, int.Parse(obj.transform.GetChild(0).name) + 14, null, () =>
                   {
                       //用于标志是否点击过展示板
        ...
        private void GameInit()
        {
            ...
            flag = 0;
            Bg.transform.GetRawImage().texture = bellTextures.texture[0];
        ...
        void TalkClick()
        {
            ...
            if (talkIndex == 1)
            {
                ...
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 13, null, () =>
                 {
                     isPlaying = false;
                     Bg.transform.GetRawImage().texture = bellTextures.texture[1];
                     flag = 0;
                     bd.SetActive(false);
                     curPageIndex = -1;
