...
namespace ILFramework.HotClass
{
    ...
    public class TD91241Part1
    {
        ...
        private void GameInit()
        {
            ...
            SpineShow.transform.GetRectTransform().anchoredPosition = new Vector3(0, -55, 0);
        ...
        void TDGameInit()
        {
            ...
            for (int i = 0; i < SpinePage.childCount; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    SpinePage.GetChild(i).GetChild(j).gameObject.GetComponent<Spine.Unity.SkeletonGraphic>().Initialize(true);
                    SpineManager.instance.DoAnimation(SpinePage.GetChild(i).GetChild(j).gameObject, SpinePage.GetChild(i).GetChild(j).GetChild(0).name+"3", false);
                }
        ...
        void TalkClick()
        {
            ...
            switch (talkIndex)
            {
                ...
                case 3:
                    ...
                    bd.SetActive(true);
                    _canClick = false;
                    mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, 10));
                    break;
            }
        ...
        private void OnClickShow(GameObject obj)
        {
            ...
            {
                mono.StartCoroutine(SpeckerCoroutine(bd, SoundManager.SoundType.VOICE, Convert.ToInt32(tem.name)+6, null,
                    () => {
