...
public class CustomImage : Image
{
    ...
    private void ApplyPathsToPolygonCollider(List<Vector2[]> paths)
    {
        ...
        for (int i = 0; i < paths.Count; i++)
        {
            ...
            {
                var v2 = path[j];
                var temp = new Vector2(v2.x * Offset, v2.y * Offset);
                path[j] = temp;
            }
    ...
    private Dictionary<Edge2D, int> BuildEdgesFromMesh()
    {
        ...
        var vertices = sp.vertices;
        var triangles = sp.triangles;

        var edges = new Dictionary<Edge2D, int>();

        for (int i = 0; i < triangles.Length - 2; i += 3)
        {
            var faceVert1 = vertices[triangles[i]];
            var faceVert2 = vertices[triangles[i + 1]];
            var faceVert3 = vertices[triangles[i + 2]];
