...
public class ScrollPage : MonoBehaviour, IBeginDragHandler, IEndDragHandler
{
    ...
    {
        isDrag = true;
        _prevPosX = eventData.position.x;
    }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        float d = 0.99f / pages.Count;
        float delta = eventData != null ? eventData.position.x < _prevPosX ? d : -d : 0f;
        for (int i = 1; i < pages.Count; i++)
        {
            float temp = Mathf.Abs(pages[i] - posX);
            float temp1 = Mathf.Abs(pages[i] - posX - delta);
            Debug.Log("offest" + offset.ToString() + "temp" + temp.ToString() + "posX" + posX.ToString() + "delta" + delta.ToString());
            if (temp1 < offset)
            {
                index = i;
        ...
        if (index!=currentPageIndex)
        {
            ...
            if (OnDragSlideLua != null) OnDragSlideLua.Call(currentPageIndex);
            if (OnDragSlide != null) OnDragSlide();
         ...
         */
        //rect.horizontalNormalizedPosition = pages[index];
